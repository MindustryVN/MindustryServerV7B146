...
public class ServerControl implements ApplicationListener {
    ...
    /**
     * Global instance of ServerControl, initialized when the server is created.
     * Should never be null on a dedicated server.
     */
    ...
    public Cons<GameOverEvent> gameOverListener = event -> {
        if (state.rules.waves) {
            info("Game over! Reached wave @ with @ players online on map @.", state.wave, Groups.player.size(),
                    Strings.capitalize(state.map.plainName()));
        } else {
            info("Game over! Team @ is victorious with @ players online on map @.", event.winner.name,
                    Groups.player.size(), Strings.capitalize(state.map.plainName()));
        }
    ...
    protected void setup(String[] args) {
        ...
        logger = (level1, text) -> {
            ...
            if (level1 == LogLevel.err)
                text = text.replace(reset, lightRed + bold);

            String result = bold + lightBlack + "[" + dateTime.format(LocalDateTime.now()) + "] " + reset
                    + format(tags[level1.ordinal()] + " " + text + "&fr");
            System.out.println(result);

            if (Config.logging.bool()) {
                logToFile("[" + dateTime.format(LocalDateTime.now()) + "] "
                        + formatColors(tags[level1.ordinal()] + " " + text + "&fr", false));
            }
        ...
        Events.run(Trigger.update, () -> {
            if (state.isPlaying() && Config.autosave.bool()) {
                if (autosaveCount.get(Config.autosaveSpacing.num() * 60)) {
                    ...
                    String mapName = (state.map.file == null ? "unknown" : state.map.file.nameWithoutExtension())
                            .replace(" ", "_");
    ...
    protected void registerCommands() {
        handler.register("help", "[command]", "Display the command list, or get help for a specific command.", arg -> {
            if (arg.length > 0) {
                ...
                if (command == null) {
                    ...
                    info(command.text + ":");
                    info("  &b&lb " + command.text + (command.paramText.isEmpty() ? "" : " &lc&fi") + command.paramText
        ...
        handler.register("version", "Displays server version info.", arg -> {
            info("Version: Mindustry @-@ @ / build @", Version.number, Version.modifier, Version.type,
        ...
        handler.register("host", "[mapname] [mode]",
                "Open the server. Will default to survival and a random map if not specified.", arg -> {
                    ...
                    Map result;
                    if (arg.length > 0) {
                        result = maps.all().find(map -> map.plainName().replace('_', ' ')
        ...
        handler.register("mods", "Display all loaded mods.", arg -> {
            if (!mods.list().isEmpty()) {
                ...
                for (LoadedMod mod : mods.list()) {
                    info("  @ &fi@ " + (mod.enabled() ? "" : " &lr(" + mod.state + ")"), mod.meta.displayName,
                            mod.meta.version);
                }
            } else {
                info("No mods found.");
        ...
        handler.register("playerlimit", "[off/somenumber]", "Set the server player limit.", arg -> {
            if (arg.length == 0) {
                info("Player limit is currently @.",
                        netServer.admins.getPlayerLimit() == 0 ? "off" : netServer.admins.getPlayerLimit());
                return;
            }
            if (arg[0].equals("off")) {
        ...
        handler.register("nextmap", "<mapname...>",
                "Set the next map to be played after a game-over. Overrides shuffling.", arg -> {
                    Map res = maps.all().find(map -> map.plainName().replace('_', ' ')
                            .equalsIgnoreCase(Strings.stripColors(arg[0]).replace('_', ' ')));
                    if (res != null) {
        ...
        handler.register("players", "List all players currently in game.", arg -> {
            if (Groups.player.size() == 0) {
                ...
                for (Player user : Groups.player) {
                    info(" @&lm @ / ID: @ / IP: @", user.admin ? "&r[A]&c" : "&b[P]&c", user.plainName(), user.uuid(),
                            user.ip());
                }
            }
        });
        ...
        handler.register("info", "<IP/UUID/name...>",
                "Find player info(s). Can optionally check for all names or IPs a player has had.", arg -> {
                    ...
                    if (infos.size > 0) {
                        ...
                        for (PlayerInfo info : infos) {
                            info("[@] Trace info for player '@' / UUID @ / RAW @", i++, info.plainLastName(), info.id,
                                    info.lastName);
     ...
     * Resets the world state, starts a new game.
     * 
     * @param wait Whether to wait for {@link Config#roundExtraTime} seconds before
     *             starting a new game.
     * @param run  What task to run to load a new world.
     */
        ...
        if (on && socketThread == null) {
            socketThread = new Thread(() -> {
                try {
                    ...
                    serverSocket.bind(
                            new InetSocketAddress(Config.socketInputAddress.string(), Config.socketInputPort.num()));
                    while (true) {
